...
Ship.prototype.draw = function(){
	...
	Game.ctx.beginPath();
	for (var i = 0; i<3; i++){
		this.tmpA = i===0 ? this.a : (this.a+180+(i==1 ? this.rearA : -this.rearA));
		this.tmpR= i===0 ? this.r : this.r*0.6;
		
		this.points[i].x = Math.sin(Math.PI/180*this.tmpA)*this.r*VAR.d+this.x;
		this.points[i].y = -Math.cos(Math.PI/180*this.tmpA)*this.r*VAR.d+this.y;
		//
		//
		...
		if(Game.key_38 && this.draw_thrust){
			...
			for(var i=0; i<3; i++){
				this.tmpA = i!=1 ? this.a+180+(i===0 ? -this.rearA+14 : this.rearA-14) : this.a+180;
				this.tmpR = i==1 ? this.r : this.r*0.5;
				//
				Game.ctx[i===0 ? 'moveTo' : 'lineTo'](
					Math.sin(Math.PI/180*this.tmpA)*this.tmpR*VAR.d+this.x,
					-Math.cos(Math.PI/180*this.tmpA)*this.tmpR*VAR.d+this.y
				);
